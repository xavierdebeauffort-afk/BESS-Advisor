...
function generateRecommendation(inputs) {
  ...
  // Check if we have minimum inputs
  ...
  const hasBTMOnly = valueStreams.every(vs => ['peakshaving', 'tou', 'selfconsumption'].includes(vs));
  ...
  // Map risk score to contract type
  if (riskScore <= 1) {
...
const BTMBatteryAdvisor = () => {
  ...
  // Get available value streams for selected region
  ...
  const copyToClipboard = () => {
    ...
    const summary = `
BTM BATTERY ADVISOR - RECOMMENDATION SUMMARY
=============================================
...
`.trim();
  ...
  // Check if inputs are complete enough for recommendation
              ...
              <div className="space-y-2">
                {Object.values(REGIONS).map(r => (
                  <RadioOption
                    ...
                    onSelect={() => {
                      setRegion(r.id);
                      setValueStreams([]); // Reset value streams when region changes
                    }}
                    title={r.name}
...
export default BTMBatteryAdvisor;